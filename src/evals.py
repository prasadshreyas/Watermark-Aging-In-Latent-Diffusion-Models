...
# Check the params.attack_mode == 'aging' for all the modifications
# 
# Adapted from  Meta's Stable Signatures paper
...
import watermark_model_utils as utils
import image_converter as utils_img_conv 
import image_processor as img_proc
import watermark_models as utils_model

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

img_converter = utils_img_conv.ImageConverter()
utils_img = img_proc.ImageProcessor()
...
def main(params):
    ...
    if params.eval_bits:
        ...
        if params.attack_mode == 'all':
            attacks = {
                'none': lambda x: x,
                'crop_05': lambda x: utils_img.crop_center(x, 0.5),
                'crop_01': lambda x: utils_img.crop_center(x, 0.1),
                'rot_25': lambda x: utils_img.rotate_image(x, 25),
                'rot_90': lambda x: utils_img.rotate_image(x, 90),
                ...
                'resize_05': lambda x: utils_img.resize_image(x, 0.5),
                'resize_01': lambda x: utils_img.resize_image(x, 0.1),
                'overlay_text': lambda x: utils_img.overlay_text(x, [76,111,114,101,109,32,73,112,115,117,109]),
                'comb': lambda x: utils_img.jpeg_compress(utils_img.adjust_brightness(utils_img.crop_center(x, 0.5), 1.5), 80),
            }
        ...
        elif params.attack_mode == 'aging':
            {
                'none': lambda x: x,
                'crop_01_1': lambda x: utils_img.crop_center(x, 0.1),
                'crop_01_2': lambda x: utils_img.crop_center(utils_img.crop_center(x, 0.1), 0.1),
                'crop_01_3': lambda x: utils_img.crop_center(utils_img.crop_center(utils_img.crop_center(x, 0.1), 0.1), 0.1),
                'crop_01_4': lambda x: utils_img.crop_center(utils_img.crop_center(utils_img.crop_center(utils_img.crop_center(x, 0.1), 0.1), 0.1), 0.1),
                'brightness_2_1': lambda x: utils_img.adjust_brightness(x, 2),
                'brightness_2_2': lambda x: utils_img.adjust_brightness(utils_img.adjust_brightness(x, 2), 2),
                'brightness_2_3': lambda x: utils_img.adjust_brightness(utils_img.adjust_brightness(utils_img.adjust_brightness(x, 2), 2), 2),
                'brightness_2_4': lambda x: utils_img.adjust_brightness(utils_img.adjust_brightness(utils_img.adjust_brightness(utils_img.adjust_brightness(x, 2), 2), 2), 2),
                'contrast_2_1': lambda x: utils_img.adjust_contrast(x, 2),
                'contrast_2_2': lambda x: utils_img.adjust_contrast(utils_img.adjust_contrast(x, 2), 2),
                'contrast_2_3': lambda x: utils_img.adjust_contrast(utils_img.adjust_contrast(utils_img.adjust_contrast(x, 2), 2), 2),
                'contrast_2_4': lambda x: utils_img.adjust_contrast(utils_img.adjust_contrast(utils_img.adjust_contrast(utils_img.adjust_contrast(x, 2), 2), 2), 2),
                'jpeg_50_1': lambda x: utils_img.jpeg_compress(x, 50),
                'jpeg_50_2': lambda x: utils_img.jpeg_compress(utils_img.jpeg_compress(x, 50), 50),
                'jpeg_50_3': lambda x: utils_img.jpeg_compress(utils_img.jpeg_compress(utils_img.jpeg_compress(x, 50), 50), 50),
                'jpeg_50_4': lambda x: utils_img.jpeg_compress(utils_img.jpeg_compress(utils_img.jpeg_compress(utils_img.jpeg_compress(x, 50), 50), 50), 50),
                'saturation_2_1': lambda x: utils_img.adjust_saturation(x, 2),
                'saturation_2_2': lambda x: utils_img.adjust_saturation(utils_img.adjust_saturation(x, 2), 2),
                'saturation_2_3': lambda x: utils_img.adjust_saturation(utils_img.adjust_saturation(utils_img.adjust_saturation(x, 2), 2), 2),
                'saturation_2_4': lambda x: utils_img.adjust_saturation(utils_img.adjust_saturation(utils_img.adjust_saturation(utils_img.adjust_saturation(x, 2), 2), 2), 2),
                'comb_1': lambda x: utils_img.jpeg_compress(utils_img.adjust_brightness(utils_img.crop_center(x, 0.5), 1.5), 80),
                'comb_2': lambda x: utils_img.jpeg_compress(utils_img.adjust_brightness(utils_img.jpeg_compress(utils_img.adjust_brightness(utils_img.crop_center(x, 0.5), 1.5), 80), 1.5), 80),
                'comb_3': lambda x: utils_img.jpeg_compress(utils_img.adjust_brightness(utils_img.jpeg_compress(utils_img.adjust_brightness(utils_img.jpeg_compress(utils_img.adjust_brightness(utils_img.crop_center(x, 0.5), 1.5), 80), 1.5), 80), 1.5), 80),
